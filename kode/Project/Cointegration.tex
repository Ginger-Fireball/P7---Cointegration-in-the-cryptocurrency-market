...
\begin{defi}{Cointegrated of order (d,b)}
    ...
    \begin{itemize}
        ...
        \item There exists a non-zero vector $\b{\beta}$ such that $\b{z}_t=\b{\beta}^\top \b{x}_t\sim I(d-b)$ where $b>0$.
    \end{itemize}
    When the above is fulfilled we denote it as $x_t\sim CI(d,b)$ and $\b{\beta}$ is called the co-integrating vector.
...
\begin{equation}
    \boldsymbol{x}_t=\Pi_1 \b{x}_{t-1} + \dots + \Pi_k \b{x}_{t-p} + \b{\mu} + \Phi \boldsymbol{d}_t +\boldsymbol{\varepsilon}_t \phantomsection \:\:\: \text{ for } \:\:\: t=1,\dots,T.
...
\begin{defi}{\hfill}
    ...
    \end{equation*}
    if there is a vector $\b{\beta}_i \neq \b{0}$ so that $\b{\beta}_i^\top\boldsymbol{x}_t$ is stationary, then $\boldsymbol{x}_t$ is cointegrated with cointegrating vector $\b{\beta}_i$. The number of linearly independent cointegrating vectors is the cointegrating rank.
\end{defi}
...
% \end{enumerate}
\noindent This insures that we do not have solutions with seasonal roots or explosive roots, and that the solutions which are of order 2 or higher is avoided. Before the The Granger Representation Theorem can be formulated a definition will be given 
\begin{defi}{\phantom{}}
if condition \ref{condition_granger} is satisfied then
    \begin{equation}
        (1-z)\Pi^{-1}(z)=C(z)=\sum^{\infty}_{i=0}C_iz^i=C(1)+(1-z)C^\ast(z).
    \end{equation}
    \begin{align*}
        \text{Where }C^{\ast}(z)=\frac{C(z)-C(1)}{1-z}\text{ for }z\neq 1.
    \end{align*} 
\end{defi}
 The Granger Representation Theorem can now be formulated as such
\begin{thm}{Granger Representation Theorem}
    ...
    \begin{equation}
C=C(1)=\beta_\perp((\alpha'_{\perp}\Gamma\beta_{\perp})^{-1}\alpha'_{\perp}
    \end{equation}
...
\end{equation}
where $\b{a}$ depends on the initial values, so that $\b{\beta'}\b{a}=0$. It follows that $\b{x}_t$ is a cointegrated $I(1)$ process with r cointegrating vectors $\beta$ and $p-r$ common stochastic trends $\b{\alpha'}_{\perp}\sum_{i=1}^{t}\b{\varepsilon}_i$.
\newline
The result \ref{Granger_representation_eq_1} rests on the observation that the singularity of $\Pi(z)$ for $z=1$ implies that $(\Pi)^{-1}$ has a pole at $z=1$. condition \ref{condition_granger} insures that the pole is of order one.
\end{thm}
\noindent The entire prove can be found in \cite{co-Integration_and_error_correction}. We will instead focus on proving the MA representation \eqref{granger_representation_eq_3}.
\begin{bevis}
...
\noindent We define the stationary process $z_t=C^\ast (L)\b{\varepsilon}_t$ and the deterministic function $\b{\mu}_t=C^\ast(L)\b{\Phi} \b{d}_t$. Then
    \begin{align*}
        \Delta \b{x}_t&=C(\b{\varepsilon}_t+\b{\Phi} \b{d}_t)+(1-L)(\b{z}_t+\b{\mu}_t)=C(\b{\varepsilon}_t+\b{\Phi} \b{d}_t)+(1-L)(\b{z}_t+\b{\mu}_t)
        \end{align*}
        Next $\b{x}_t$ will be derived iterative
        \begin{align*}
    ...
    \end{align*}
    The equations above cumulates, where we get the sum over $C(1)(\b{\varepsilon}_i+\b{\Phi} \b{d}_i)$ and $\b{z}_i-\b{z}_{i-1}+\b{\mu}_i-\b{\mu}_{i-1}$ which is a telescoping sum. Furthermore we define $\b{a}=\b{x}_0-\b{z}_0-\b{\mu}_0$, then $\b{x}_t$ can be written as
\begin{align}
    \b{x}_t=C\sum^{t}_{i=1}(\b{\varepsilon}_i+\b{\Phi} \b{d}_i)+\b{z}_t+\b{\mu}_t+\b{a}
\end{align}
Here \eqref{eq:moving_average_representation} can be used to write the infinite MA representation of $\b{z}_t+\b{\mu}_t$ this gives us
...
\end{align} 
We choose the distribution of $\b{x}_0$ so that $\beta'\b{x}_0=\beta(\b{z}_0+\b{\mu}_0)$, and here $\beta'\b{a}=0$. It is seen that $\b{x}_t$ is $I(1)$, that $\beta'\b{x}_t=\beta'\b{z}_t+\beta'\b{\mu}_t$, so that $\beta'\b{x}_t$ is stationary around its mean $E(\beta'\b{x}_t)=\beta'\b{\mu}_t$ and that $\Delta \b{x}_t$ is stationary around its mean $E(\Delta \b{x}_t) =C\b{\Phi} \b{d}_t +\Delta\b{\mu}_t$. This concludes the proof
...
Consider the unrestricted hypothesis $H_1$ which is the model  \eqref{eq:p-dimensional_Var_model}. The various hypotheses that are given in Table \ref{tab:Various_Hypotheses} are related to the existence of cointegrating relations combined with linear restrictions.
\begin{table}[H]
    \centering
    \begin{tabular}{l}
...
\end{table}
\noindent The case where the non-stationary process is experiencing an absence of a linear trend. The augmented hypotheses are used, $H^\ast_j$ for $j=2,\dots,5$, which are augmented by $\b{\mu}=\alpha\beta^\top_0$, this is equivalent to $\alpha^\top_\perp \b{\mu}=0$. The matrices $A\in \R^{k\times m}$ and $H\in \R^{k\times s}$ are known and define a linear restrictions on the parameters $\alpha$ and $\beta$. The restrictions reduce the parameters to $\varphi \in \R^{s \times r}$ and $\psi \in \R^{m \times r}$, where $r \leq s \leq k$ and $r \leq m \leq k$.\\

\noindent The main hypothesis of interest is $H_2$. It should be noted that for each value of $r$, there is a corresponding hypothesis $H_j(r)$ of $r$ or fewer cointegrating relations. The analysis makes it possible to conduct inferences about the number of cointegrating relationships $(r)$ through hypothesis testing. It can be tested that $H_2(r)$ in $H_1$ or by testing if an increase of one in the number of cointegrating vectors improves the model, $H_2(r)$ in $H_2(r+1)$. The last approach forms a nested sequence of the models $H(r)$
