...
Consider the unrestricted hypothesis $H_1$ given in \eqref{eq:p-dimensional_Var_model}. The various hypotheses that are given in Table \ref{tab:Various_Hypotheses} are related to the existence of cointegration relations combined with linear restrictions.
\begin{table}[H]
    \centering
    \begin{tabular}{l}
...
\end{equation*}
where the processes $H(r)$ contains the processes with cointegration equal to or less than $r$. The formulation above allows for the derivation of the likelihood ratio test for hypothesis $H(r)$ in $H(k)$. These tests can then be applied to check if prior knowledge of the number of cointegration relations is consistent with the data or to construct an estimator of the cointegration rank.
